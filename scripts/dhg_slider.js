...
var form_amount_slider_handle = 'images/slider_handle.gif';
var slider_handle_image_obj = false;
var sliderObjectArray = [];
var slider_counter = 0;
var slideInProgress = false;
var handle_start_x;
var event_start_x;
var currentSliderIndex;
var sliderHandleWidth = 18;

function resetSliderControl(){
	slider_handle_image_obj = false;
	sliderObjectArray = [];
	slider_counter = 0;
...
function getImageSliderHeight(){
	if(!slider_handle_image_obj){
		...
		slider_handle_image_obj.src = form_amount_slider_handle;
	}
...
function positionSliderImage(e,theIndex){
	...
	if(!theValue.match(/^-?[0-9.]*$/g)){
		if(sliderObjectArray[theIndex]['min'] <= 0 && sliderObjectArray[theIndex]['max'] >= 0){
			theValue=0;
		}else{
...
sliderPreloadedImages[0].src = form_amount_slider_handle;

function form_amount_slider(targetElId,formTarget,width,min,max,onchangeAction){
	if(!slider_handle_image_obj){
	...
	parentObj.style.height = '48px';	// The height of the image
	parentObj.style.position = 'relative';
	parentObj.style.top = '0';
	parentObj.style.left = '0';
	parentObj.style.cssFloat= 'left';
	parentObj.style.margin = '0 10px';
	parentObj.id = 'slider_container' + slider_counter;
	document.getElementById(targetElId).appendChild(parentObj);

	var obj = document.createElement('DIV');
	obj.className = 'form_amount_slider';
