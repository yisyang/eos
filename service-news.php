...
<?php
	...
	$sql = "SELECT queue_res.id, queue_res.pid, queue_res.newlevel, queue_res.starttime, queue_res.endtime, list_prod.name, list_prod.has_icon, list_prod.res_cost, list_prod.tech_avg FROM queue_res LEFT JOIN list_prod ON queue_res.pid = list_prod.id WHERE queue_res.frid = $bldg_id AND queue_res.endtime >= $timenow ORDER BY queue_res.starttime ASC";
					...
					if(cd_remaining_fbox > 15){
						...
						if(cd_remaining_fbox <= 0){
							if(!reloading_fbox){
								...
								// setTimeout(function(){
									// jqDialogInit('rnd-res.php?frid=<?= $bldg_id ?>');
								// }, 1500);
								return;
							}
						}
			...
			function cancelQueue(queue_id){
				progressController.cancelQueue('rnd', queue_id, function(){
					progressController.refreshQueue('rnd', function(resp){
						...
						if(qr_count){
							var queue_res_list = '';
							var queue_res_last_frid = 0;
							for(var i = 1; i < qr_count; i++){
								var queue_res_item = resp.queue_res[i];
